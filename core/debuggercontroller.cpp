...
bool DebuggerController::SaveCodeCoverageToFile(const std::filesystem::path& filePath) const
{
	...
	try
	{
		...
		if (!file.is_open())
		{
			LogErrorF("Failed to open file for writing: {}", filePath);
			return false;
		}
		...
		file.close();
		LogInfoF("Saved {} executed instruction addresses to {}", count, filePath);
...
bool DebuggerController::LoadCodeCoverageFromFile(const std::filesystem::path& filePath)
{
	try
	{
		...
		if (!file.is_open())
		{
			LogErrorF("Failed to open file for reading: {}", filePath);
			return false;
		}
		...
		LogInfoF("Loaded {} executed instruction addresses from {}", count, filePath);
		return true;
	}
	catch (const std::exception& e)
	{
