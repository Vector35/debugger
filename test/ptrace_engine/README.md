...
| `fault_driver.cpp`, `faultwrap.cpp`, `run_faults.sh` | Inject failures into ptrace, memory writes, and target-memory setup to check that operations fail transactionally. `faultwrap.cpp` wraps `ptrace`, `pwrite`, `open` and `waitpid` at link time: it can fail the next call of one request (`PtraceTestFailNextRequest`), fail the Nth write (`PtraceTestFailPwriteAfter`), and hide a task's statuses from `waitpid` (`PtraceTestHideWaitpid`), which makes a thread look like one that never stops. `./fault_driver <name>` runs one test. The timeout tests (`stopall_timeout`, `kill_timeout`, `detach_timeout`, `launch_timeout`, `attach_timeout`) and the error tests (`geteventmsg_failure`, `endguard_failure`, `hw_apply_failure`, `pc_rewind_failure`, x86 only) are here. |
