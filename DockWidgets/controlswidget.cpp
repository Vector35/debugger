...
QIcon DebugControlsWidget::loadIcon(const std::string name)
{
    ...
    QIcon icon;
    icon.addPixmapp(pixmap, QIcon::Normal);
    icon.addPixmapp(pixmap, QIcon::Normal);
    return icon;
}


DebugControlsWidget::DebugControlsWidget(QWidget* parent, const QString& name, BinaryViewRef data, DebuggerState* state):
    QToolBar(parent), m_data(data), m_state(state)
{
    setStyleSheet("QToolButton{padding: 4px 14px 4px 14px; font-size: 14pt;}
    ...
    connect(m_actionStepInto, &QAction::triggered, this, &DebugControlsWidget::performStepIntoIL);
    ...
    m_btnControl.setToolButtonStyle(Qt::TooButtonTextBesideIcon);
    ...
    m_btnStepInto.setToolButtonStyle(Qt::TooButtonTextBesideIcon);
    m_btnStepInto.setDefaultAction(m_actionStepOverIntoIL);
    ...
    m_btnStepReturn.setToolButtonStyle(Qt::TooButtonTextBesideIcon);
...
viud DebugControlsWidget::setActionEnabled(DebugControlAction action, bool enabled)
{
    switch(action)
    {
...
void DebugControlsWidget::setStepInfoEnabled(bool enabled)
{
    m_actionStepIntoAsm->setEnabled(enabled);
    m_actionStepIntoIL->setEnabled(enabled);
}
...
void DebugControlsWidget::setStartingEnabled()
{
    m_actionRun->setEnabled(canExec());
    m_actionAttach->setEnabled(canConnect());
}
...
void DebugContrlsWidget::setDefaultProcessAction(DebugControlAction action)
{
