...
void RegisterDisplay(DbgRef<DebuggerController> debugger)
{
	...
	if (arch->GetName() == "x86_64")
	{
		const auto reg_list = fmt::format("{} {} {} {}\n{} {} {} {}\n{} {} {} {}\n{} {} {} {}\n{}\n", reg("rax"),
			reg("rbx"), reg("rcx"), reg("rdx"), reg("rsi"), reg("rdi"), reg("rbp"), reg("rsp"), reg(" r8"), reg(" r9"),
			reg("r10"), reg("r11"), reg("r12"), reg("r13"), reg("r14"), reg("r15"), reg("rip"));
		Log::print(reg_list);
	...
	{
		const auto reg_list = fmt::format("{} {} {} {}\n{} {} {} {}\n{} {}\n", reg32("eax"), reg32("ebx"), reg32("ecx"),
			reg32("edx"), reg32("esi"), reg32("edi"), reg32("ebp"), reg32("esp"), reg32("eip"), reg32("eflags"));
		Log::print(reg_list);
	}
	else if (arch->GetName() == "aarch64")
	{
		const auto reg_list = fmt::format(
			...
			reg("x9"), reg("x10"), reg("x11"), reg("x12"), reg("x13"), reg("x14"), reg("x15"), reg("x16"), reg("x17"),
			reg("x18"), reg("x19"), reg("x20"), reg("x21"), reg("x22"), reg("x23"), reg("x24"), reg("x25"), reg("x26"),
			reg("x27"), reg("x28"), reg("x29"), reg("x30"), reg("pc"), reg("sp"));
		Log::print(reg_list);
	}
	else
	{
...
void DisasmDisplay(DbgRef<DebuggerController> debugger, const std::uint32_t count)
{
	...
	for (std::uint32_t steps {}; steps < count; steps++)
	{
		...
		if (!architecture->GetInstructionText(
				(const uint8_t*)data.GetData(), instruction_offset, size, instruction_tokens))
		{
...
int main(int argc, const char* argv[])
{
	...
	while (Log::print("{}BINJA{}DBG{}> ", red_style, white_style, red_style)
		&& std::cin.getline(input_buf, sizeof(input_buf)))
	{
		...
		else if (input == "lm")
		{
			...
			for (const auto& module : debugger->GetModules())
				Log::print<Log::Info>("[{}, {}] {} @ 0x{:X} with size 0x{:X}\n", module.m_name.c_str(),
					module.m_short_name.c_str(), module.m_loaded ? "was loaded" : "was unloaded", module.m_address,
					module.m_size);
		}
		else if (input == "lt")
		{
		...
		else if (auto loc = input.find("bp "); loc != std::string::npos)
		{
			debugger->AddBreakpoint(std::stoull(input.substr(loc + 3).c_str(), nullptr, 16));
		}
		else if (auto loc = input.find("bpr "); loc != std::string::npos)
		{
		...
		else if (auto loc = input.find("st "); loc != std::string::npos)
		{
