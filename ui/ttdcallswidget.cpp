...
void TTDCallsQueryWidget::setupUIActions()
{
	...
	m_actionHandler.bindAction("Add TTD Bookmark...", UIAction([&]() {
		...
		if (row < 0)
			return;
		QTableWidgetItem* posItem = m_resultsTable->item(row, TimeStartColumn);
		if (!posItem)
			return;
		QString posStr = posItem->text();

		// Use function address as view address
		uint64_t viewAddress = 0;
		QTableWidgetItem* addrItem = m_resultsTable->item(row, FunctionAddressColumn);
		if (addrItem && addrItem->text().startsWith("0x"))
		{
			bool ok;
			viewAddress = addrItem->text().mid(2).toULongLong(&ok, 16);
			if (!ok)
				viewAddress = 0;
		}

		// Use function name as default note
		QString defaultNote;
		QTableWidgetItem* funcItem = m_resultsTable->item(row, FunctionColumn);
		if (funcItem)
